...
CREATE CLUSTERED INDEX cix_131SPolicy_UPolId
    ON Staging.AMS_131SPolicy (UPolId);

CREATE CLUSTERED INDEX cix_131SPolicy_UPolId
    ON RawData.AMS_131SPolicy (UPolId);
GO
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_131SPolicy
     WHERE  EXISTS (
                       SELECT   *
                        ...
                        WHERE   RawData.UPolId = AMS_131SPolicy.UPolId
                   );

    INSERT INTO Staging.AMS_131SPolicy (SPolicyChecksum
                                        , Datasource
                                        , Deleted
                                        , Sequencenumber
                                        , ChangedBy
    ...
    SELECT  AMS_131SPolicy.SPolicyChecksum
      ...
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_131SPolicy.UPolId
