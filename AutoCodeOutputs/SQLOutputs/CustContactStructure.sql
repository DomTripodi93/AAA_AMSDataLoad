...
CREATE CLUSTERED INDEX cix_CustContact_Company_CustContactId ON Staging.AMS_CustContact(Company, CustContactId)
CREATE CLUSTERED INDEX cix_CustContact_Company_CustContactId ON RawData.AMS_CustContact(Company, CustContactId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_CustContact
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.CustContactId = AMS_CustContact.CustContactId
                   );

    INSERT INTO Staging.AMS_CustContact (
        CustContactChecksum
		, Datasource
		, Deleted
		, Sequencenumber
    ...
    --FROM  RawData.AMS_CustContact;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_CustContact.CustContactId
