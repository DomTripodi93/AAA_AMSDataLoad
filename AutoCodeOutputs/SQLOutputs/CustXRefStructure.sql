...
CREATE CLUSTERED INDEX cix_CustXRef_Company_CustXRefId ON Staging.AMS_CustXRef(Company, CustXRefId)
CREATE CLUSTERED INDEX cix_CustXRef_Company_CustXRefId ON RawData.AMS_CustXRef(Company, CustXRefId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_CustXRef
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.CustXRefId = AMS_CustXRef.CustXRefId
                   );
    ...
    --FROM  RawData.AMS_CustXRef;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_CustXRef.CustXRefId
