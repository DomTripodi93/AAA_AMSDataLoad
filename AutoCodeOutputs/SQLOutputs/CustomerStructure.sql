...
CREATE CLUSTERED INDEX cix_Customer_Company_CustomerId ON Staging.AMS_Customer(Company, CustomerId)
CREATE CLUSTERED INDEX cix_Customer_Company_CustomerId ON RawData.AMS_Customer(Company, CustomerId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_Customer
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.CustomerId = AMS_Customer.CustomerId
                   );

    INSERT INTO Staging.AMS_Customer (
        CustomerChecksum
		, Datasource
		, Deleted
		, Sequencenumber
    ...
    --FROM  RawData.AMS_Customer;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_Customer.CustomerId
