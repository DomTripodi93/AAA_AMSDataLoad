...
CREATE CLUSTERED INDEX cix_BasicPolInfo_Company_BasicPolInfoId ON Staging.AMS_BasicPolInfo(Company, BasicPolInfoId)
CREATE CLUSTERED INDEX cix_BasicPolInfo_Company_BasicPolInfoId ON RawData.AMS_BasicPolInfo(Company, BasicPolInfoId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_BasicPolInfo
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.BasicPolInfoId = AMS_BasicPolInfo.BasicPolInfoId
                   );
    ...
    --FROM  RawData.AMS_BasicPolInfo;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_BasicPolInfo.BasicPolInfoId
