...
CREATE CLUSTERED INDEX cix_130Policy_Company_130PolicyId ON Staging.AMS_130Policy(Company, 130PolicyId)
CREATE CLUSTERED INDEX cix_130Policy_Company_130PolicyId ON RawData.AMS_130Policy(Company, 130PolicyId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_130Policy
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.130PolicyId = AMS_130Policy.130PolicyId
                   );
    ...
    --FROM  RawData.AMS_130Policy;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_130Policy.130PolicyId
