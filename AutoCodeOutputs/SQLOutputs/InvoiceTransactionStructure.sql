...
CREATE CLUSTERED INDEX cix_InvoiceTransaction_Company_InvoiceTransactionId ON Staging.AMS_InvoiceTransaction(Company, InvoiceTransactionId)
CREATE CLUSTERED INDEX cix_InvoiceTransaction_Company_InvoiceTransactionId ON RawData.AMS_InvoiceTransaction(Company, InvoiceTransactionId)
...
BEGIN
    ...
    DELETE  FROM Staging.AMS_InvoiceTransaction
     WHERE  EXISTS (
                       SELECT   *
                         ...
                         WHERE   RawData.InvoiceTransactionId = AMS_InvoiceTransaction.InvoiceTransactionId
                   );

    INSERT INTO Staging.AMS_InvoiceTransaction (
        InvoiceTransactionChecksum
		, Datasource
		, Deleted
		, Sequencenumber
		, BackInvTPId
    ...
    --FROM  RawData.AMS_InvoiceTransaction;
      FROM  (
                SELECT  *
                        , ROW_NUMBER () OVER (PARTITION BY AMS_InvoiceTransaction.InvoiceTransactionId
